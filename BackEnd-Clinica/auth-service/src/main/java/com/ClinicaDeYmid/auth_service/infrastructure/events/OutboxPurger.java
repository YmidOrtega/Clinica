...
class OutboxPurger {
    ...
    private static final String DELETE_EXPIRED = "DELETE FROM " + JdbcAuthEventOutbox.TABLE + " WHERE created_at < ? ORDER BY created_at LIMIT ?";
    ...
    @Scheduled(initialDelayString = "PT1M", fixedDelayString = "${clinica.auth.outbox.purge-interval:PT1H}")
    int purgeExpired() {
