...
public class PasswordRecovery {
    ...
    public PasswordRecovery(Users users, MailOutbox outbox, OneTimeTokens tokens, PasswordPolicy passwordPolicy, PasswordHasher hasher,
                            LoginThrottle throttle, SessionRevocation sessions, TransactionOperations transactions, Clock clock) {
        this.users = users;
        this.outbox = outbox;
        this.tokens = tokens;
    ...
    public void request(String email) {
        ...
        transactions.executeWithoutResult(status -> users.findByEmail(address)
                .filter(MailKind.PASSWORD_RESET::appliesTo)
                .filter(user -> !outbox.hasPending(MailKind.PASSWORD_RESET, user.uuid()))
